...
public class ResourceFamilyArea extends AbstractArea
{
			...
			int yPos_, boolean firstArea_)
	{
		...
		for (GenericItem item : manaItems)
		{
			...
			TextComponent currentStock = new TextComponent(10, increment);
			TextComponent desiredStock = new TextComponent(10, increment + 3);
	...
	public void update()
	{
		...
		for (String itemTag : items)
		{
			...
			Map<String, Integer> itemFamily = _model.getItemInventory().get(
					item.getItemFamily());
 			currentStock.setText("" + itemFamily.get(itemTag));
		}
		// _components.clear();
		// if (_firstArea)
		// {
